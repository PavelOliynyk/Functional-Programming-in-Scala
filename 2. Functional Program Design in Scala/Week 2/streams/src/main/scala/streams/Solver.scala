...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filterNot { case (b, ms) => explored contains b }
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = initial match {
    ...
    case (b, ms) #:: rest => {
      ...
      val m = newNeighborsOnly(neighborsWithHistory(b, ms), explored + b)

      m ++ from(rest ++ m, explored + b)
    }
